...
describe('ExplorerWorkspace', () => {
  ...
  it('presents prompt epochs without repeating unchanged resume snapshots', () => {
    ...
    expect(within(initialCard as HTMLElement).getByText('EPOCH 01')).toBeTruthy()
    expect(resumeCard?.textContent).toContain('SYSTEM PROMPT, MODEL, AND TOOLS UNCHANGED FROM EPOCH 01')
    ...
    fireEvent.click(within(initialCard as HTMLElement).getByRole('button', { name: 'View raw request header epoch 01' }))
    expect(selected).toEqual(['root:10'])
    expect(screen.getByText('CONTEXT · EPOCH 01')).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: 'View effective context epoch 01' }))
