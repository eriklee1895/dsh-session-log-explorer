...
- Publish `@eriklee1895/dsh-session-log-explorer` only through `.github/workflows/publish.yml`. The workflow accepts `v*` tags, verifies the tag matches `package.json`, runs `pnpm check`, and uses npm Trusted Publishing through GitHub OIDC.
